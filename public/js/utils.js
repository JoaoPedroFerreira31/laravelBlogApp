...
window.localForage = (localforage__WEBPACK_IMPORTED_MODULE_1___default());

/* Set item in localForage */
window.saveStorage = function (name, data) {
  localforage__WEBPACK_IMPORTED_MODULE_1___default().setItem(name, data).then(function (value) {
    return true;
  })["catch"](function (err) {
    console.log(err);
    return false;
...
window.date_short = function (date) {
  return luxon__WEBPACK_IMPORTED_MODULE_0__.DateTime.fromISO(date).toLocaleString({
    month: 'numeric',
    year: 'numeric',
    day: 'numeric'
  });
